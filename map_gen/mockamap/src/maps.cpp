...
Maps::randomMapGenerate()
{
  ...
  GET_PARAM(info.nh_private, "width_min", _w_l, 0.6);
  GET_PARAM(info.nh_private, "width_max", _w_h, 1.5);
  GET_PARAM(info.nh_private, "obstacle_number", _ObsNum, 10);
...
Maps::perlin3D()
{
  ...
  GET_PARAM(info.nh_private, "complexity", complexity, 0.142857);
  GET_PARAM(info.nh_private, "fill", fill, 0.38);
  GET_PARAM(info.nh_private, "fractal", fractal, 1);
  GET_PARAM(info.nh_private, "attenuation", attenuation, 0.5);
...
Maps::maze2D()
{
  ...
  int    addWallX;
  int    addWallY;
  GET_PARAM(info.nh_private, "road_width", width, 1.0);
  GET_PARAM(info.nh_private, "add_wall_x", addWallX, 0);
  GET_PARAM(info.nh_private, "add_wall_y", addWallY, 0);
  GET_PARAM(info.nh_private, "maze_type", type, 1);
...
Maps::Maze3DGen()
{
  ...
  GET_PARAM(info.nh_private, "numNodes", numNodes, 10);
  GET_PARAM(info.nh_private, "connectivity", connectivity, 0.5);
  GET_PARAM(info.nh_private, "nodeRad", nodeRad, 3);
  GET_PARAM(info.nh_private, "roadRad", roadRad, 2);
